	...
	private List<Node> buildtriehelper(String[] patterns) {
		...
		for(String pattern : patterns)
		{
			...
			for(int i = 0;i<pattern.length();i++)
			{
              ...
              if(i == pattern.length()-1)  //build the trie of patterns and at the end of each pattern mark it validword = true so that we can searchthe prefixes as well as the full text pattern
              {
